...
public interface MethodMatcher {
	...
	/**
	 * 判断方法匹配器是动态的还是静态的，true->动态，false->静态
	 * 当切入点是静态时，spring会针对目标上的每一个方法调用一次MethodMatch的{@link MethodMatcher#matches(Method, Class)},其返回值会被缓存起来，日后调用这个方法时会直接取缓存值
	 * 当切入点是动态时，spring会先调用{@link MethodMatcher#matches(Method, Class)}进行一次基础性检查，如果返回为true，
	 * 则再进行{@link MethodMatcher#matches(Method, Class,Object[])}方法的调用，一个动态的MethodMatcher可以根据一次具体的方法调用，而不是方法本身，来决定切入点是否可用
