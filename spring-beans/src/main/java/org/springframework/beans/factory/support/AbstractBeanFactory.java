...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 * @param args arguments to use when creating a bean instance using explicit arguments
	 * (only applied when creating a new instance as opposed to retrieving an existing one) 构造方法参数
	 * @param typeCheckOnly whether the instance is obtained for a type check,
	 * not for actual use 是否只检查类型是否匹配
	 * @return an instance of the bean
	 * @throws BeansException if the bean could not be created
	 */
	...
	protected <T> T doGetBean(
		...
		if (sharedInstance != null && args == null) {
			...
			// 通过bean实例获取用户真正需要的bean实例
			// 主要是为了处理当前bean实现了FactoryBean接口的情况
			...
			// 如果非单例模式下的bean正在创建，这里又开始创建，表明存在循环依赖，直接抛出异常
			...
			// 如果不是只检测类型是否匹配，这里要标记bean为已处理，因为马上就要开始创建了
			if (!typeCheckOnly) {
			...
			try {
				...
				// 获取bean的所有依赖bean，要先保证bean的依赖先初始化了
				String[] dependsOn = mbd.getDependsOn();
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						// 检测是否存在循环依赖，如果存在则抛出异常
				...
				// 如果是单例bean
				if (mbd.isSingleton()) {
					// 获取bean，这里也是一个getSingleton，跟之前那个getSingleton方法差不多
					// 这里的getSingleton方法会先尝试获取bean，如果获取不到bean的情况下会使用lambda表达式去创建一个
					sharedInstance = getSingleton(beanName, () -> {
						try {
							...
							// 如果创建过程中出现异常，则显式从缓存中删除当前bean相关信息
							// 在单例模式下为了解决循环依赖，创建过程中会缓存早期对象，这里需要进行删除
				...
				// 如果是多例的bean，那么每次去获取都要去创建一个新的实例
				else if (mbd.isPrototype()) {
					// It's a prototype -> create a new instance.
					Object prototypeInstance = null;
					try {
						beforePrototypeCreation(beanName);
						// 可以看到这里是直接去调用createBean去创建bean了
				...
				// 自定义scope类型，像springMVC的session、request那些scope就是走的这段逻辑
				else {
		...
		// 类型转换
		// 如果requireType不为空，且bean不是requireType的类型，则需要将bean转成requireType类型
	...
	public boolean containsBean(String name) {
		...
		if (containsSingleton(beanName) || containsBeanDefinition(beanName)) {
			//如果不是工厂bean产生的实例或者本身就是工厂bean就返回true
	...
	public boolean isSingleton(String name) throws NoSuchBeanDefinitionException {
		...
		if (beanInstance != null) {
			...
			if (beanInstance instanceof FactoryBean) {
				//要求是工厂bean产生的实例或者这个工厂bean是单例
				return (BeanFactoryUtils.isFactoryDereference(name) || ((FactoryBean<?>) beanInstance).isSingleton());
			}
			else {
				//否则要求这个bean不是工厂bean产生的实例
		...
		if (mbd.isSingleton()) {
			...
			if (isFactoryBean(beanName, mbd)) {
				...
				//取对应的工厂，看对应的工厂是否为单例bean
				FactoryBean<?> factoryBean = (FactoryBean<?>) getBean(FACTORY_BEAN_PREFIX + beanName);
				return factoryBean.isSingleton();
			}
			else {
				//如果不是工厂bean，因为根bean定义是单例，所以只要不是工厂bean产生的实例就可以认为是单例了
				return !BeanFactoryUtils.isFactoryDereference(name);
			}
		}
		else {
	...
	protected boolean isTypeMatch(String name, ResolvableType typeToMatch, boolean allowFactoryBeanInit)
		...
		// 如果是isTypeMatch(String,Class)调的这个方法，这里永不为空
		Class<?> classToMatch = typeToMatch.resolve();
		...
		/**
		 * 用于类加载，因为spring有自己的一套加载机制{@link DecoratingClassLoader},必须要保证FactoryBean、typeMatch这两个Class的加载器一致
		 * 不然isInstance就没有任何意义了
		 */
	...
	/**
	 * 获取类型转换器，如果没有自定义的类型转换器
	 * 则去默认的，默认的是TypeConverterDelegate
	 * @return
	 */
	@Override
	...
	public BeanDefinition getMergedBeanDefinition(String name) throws BeansException {
		...
		//若bean定义不存在，且父容器是ConfigurableBeanFactory的实例，则让父容器来调用这个方法
	...
	/**
	 * 与当前线程绑定，如果当前线程没有添加到任何bean，就直接把beanName和当前线程绑定，
	 * 否则就把beanName和以前的beanName组成一个set集合去绑定当前的线程
	...
	protected RootBeanDefinition getMergedBeanDefinition(
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			// 若给定的Bean定义并没有包含Bean定义,直接根据beanName取根定义
			...
			if (mbd == null || mbd.stale) {
				previous = mbd;
				// 若Bean没有定义父类则直接转换成RootBeanDefinition对象
				if (bd.getParentName() == null) {
					...
					try {
						...
						// 若Bean名称和父Bean的名称不一样，则根据父Bean定义名称来返回合并后的bean定义
	...
	/**
	 * 把RootBeanDefinition中的beanClassName加载为Class类
	 * @param mbd
	 * @param typesToMatch
	 * @return
	...
	private Class<?> doResolveBeanClass(RootBeanDefinition mbd, Class<?>... typesToMatch)
		...
		/**
		 * Spring自定义的类型加载器DecoratingClassLoader修改了JDK类加载规则，自己先加载一把，没有再委派给父加载器
		 * 这就产生了一个问题，每个临时的类加载器可能加载同一个类可能出现多个
		 * 可以将其加入到excludeClass扔采用双亲委派
		 */
		...
		// 如果className含有占位符，解析后发生了变化，则不用缓存，以后每次都解析一次
		if (className != null) {
			// 解析EL表达式
	...
	protected Class<?> predictBeanType(String beanName, RootBeanDefinition mbd, Class<?>... typesToMatch) {
		...
		// 工厂方法一律返回null，子类会重载后解析对应的getFactoryMethodName
	...
	protected ResolvableType getTypeForFactoryBean(String beanName, RootBeanDefinition mbd, boolean allowInit) {
		...
		if (allowInit && mbd.isSingleton()) {
			try {
				// 先创建FactoryBean实例，然后再调用其getObjectType方法获取真实类型(这代价也太昂贵了)
	...
	/**
	 * 如果是工厂bean，则返回其工厂方法返回的实例，否则返回其本身
	...
	protected Object getObjectForBeanInstance(
		...
		//如果name不符合工厂Bean的格式且不是工厂Bean的实例则直接返回
		if (!(beanInstance instanceof FactoryBean)) {
			return beanInstance;
		}
		//剩下的逻辑就是name不符合工厂bean的格式且beanInstance是工厂bean的情况
		Object object = null;
		if (mbd != null) {
			mbd.isFactoryBean = true;
		}
		else {
			//尝试从factoryBeanObjectCache缓存中获取
		...
		if (object == null) {
			...
			//调用FactoryBeanRegistrySupport.getObjectFromFactoryBean方法获取工厂Bean
