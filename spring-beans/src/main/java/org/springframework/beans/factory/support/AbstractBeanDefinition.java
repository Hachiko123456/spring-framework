...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 用来表示一个bean的实例化依靠另一个bean先实例化，对应bean属性depends-on
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * 对应bean属性autowire-candidate，当该属性设置为false时，这样容器在查找该自动装配对象时，将不考虑
	 * 该bean，即它不会被考虑作为其他bean自动装配的候选者，但是该bean本身还是可以使用自动装配来注入该bean
	 */
	...
	/**
	 * 替代工厂方法或者构造器创建对象，不影响其后面的生命周期
	 * 有这个属性的好处是，不管是静态工厂还是工厂方法，都需要通过反射调用目标方法去创建对象，反射或多或少都会影响性能
	 * 用java8的函数式编程可以避免反射带来的开销，直接调用回调方法即可
	 */
	@Nullable
	...
	/**
	 * 返回此bean被@Qualifier或者@Qualifier派生注解的属性值
