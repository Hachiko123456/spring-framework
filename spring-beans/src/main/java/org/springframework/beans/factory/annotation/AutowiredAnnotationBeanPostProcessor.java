...
public class AutowiredAnnotationBeanPostProcessor implements SmartInstantiationAwareBeanPostProcessor,
	...
	/**
	 * 把Class以及在Class中被Autowire和Value注解的属性批量封装成InjectMetadata
	 * @param beanName
	 * @param clazz
	 * @param pvs
	 * @return
	...
	private InjectionMetadata findAutowiringMetadata(String beanName, Class<?> clazz, @Nullable PropertyValues pvs) {
		...
		//获取bean的缓存key，beanName为空则使用class的名称
		String cacheKey = (StringUtils.hasLength(beanName) ? beanName : clazz.getName());
		// Quick check on the concurrent map first, with minimal locking.
		//从key中查找缓存里面的元数据
		InjectionMetadata metadata = this.injectionMetadataCache.get(cacheKey);
		//metadata为空或者clazz!=metadata.targetClass
	...
	/***
	 * 把clazz和clazz的父类中被@Autowire、@inject、@Value注解的属性和方法封装成InjectionMetadata
	 * @param clazz
	 * @return org.springframework.beans.factory.annotation.InjectionMetadata
	 **/
	private InjectionMetadata buildAutowiringMetadata(final Class<?> clazz) {
		/**
		 * 1.如果autowiredAnnotationTypes有java原生的注解则返回true
		 * 2.如果clazz是java原生的class或者Order.class的实例则返回false
		 * 3.返回true
		 */
		...
		do {
			...
			//处理Autowire和Value注解的属性
			ReflectionUtils.doWithLocalFields(targetClass, field -> {
				//查找Autowire注解的Field
				MergedAnnotation<?> ann = findAutowiredAnnotation(field);
				if (ann != null) {
					//Autowire和Value注解不支持静态属性
					...
					//解析Autowire注解的require属性
					boolean required = determineRequiredStatus(ann);
					currElements.add(new AutowiredFieldElement(field, required));
				}
			});

			//处理Autowire和Value注解的方法
			ReflectionUtils.doWithLocalMethods(targetClass, method -> {
