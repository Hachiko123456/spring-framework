...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	/**
	 * 当前对象所属嵌套层次的属性名，最顶层为空
	 */
	private String nestedPath = "";

	/**
	 * 被包装的对象的父对象(eg: rootObject.wrappedObject.property)
	 */
	@Nullable
	Object rootObject;
	...
	/**
	 * 当前嵌套属性的nestedPath和对应的AbstractNestablePropertyAccessor对象
	 */
	@Nullable
	...
	protected Object getPropertyValue(PropertyTokenHolder tokens) throws BeansException {
		//标准的嵌套属性名称，如 attrs['key'][0] 处理成 attrs[key][0]
		String propertyName = tokens.canonicalName;
		//属性名称，如嵌套属性 attrs['key'][0] 在 bean 中的属性名称为 attrs
		String actualName = tokens.actualName;
		//获取当前属性的属性处理器
		PropertyHandler ph = getLocalPropertyHandler(actualName);
		//属性处理器为空，或者属性不可读，则抛出异常
		...
		try {
			...
			if (tokens.keys != null) {
				//如果属性对应的值为null
				if (value == null) {
					//是否允许自动增长的包含null的嵌套路劲，不允许就会抛出异常
					if (isAutoGrowNestedPaths()) {
	...
	/**
	 * 根据nestedPath获取新的AbstractNestablePropertyAccessor
	...
	protected AbstractNestablePropertyAccessor getPropertyAccessorForPropertyPath(String propertyPath) {
		//找出第一个嵌套属性的下标 eg:director.info.name 返回director
		int pos = PropertyAccessorUtils.getFirstNestedPropertySeparatorIndex(propertyPath);
		// Handle nested properties recursively.
		/**
		 * 递归处理嵌套属性 eg:director.info.name
		 * 1.先处理director属性属性所在类的rootBeanWrapper
		 * 2.再获取info属性所在类的directorBeanWrapper
		 * 3.以此类推，获取最后一个属性name属性所在类的infoBeanWrapper
		 */
		if (pos > -1) {
			//属性名
	...
	private AbstractNestablePropertyAccessor getNestedPropertyAccessor(String nestedProperty) {
		...
		//把属性的属性名、下标等信息解析成PropertyTokenHolder
		PropertyTokenHolder tokens = getPropertyNameTokens(nestedProperty);
		//标准的嵌套属性名称，如 attrs['key'][0] 处理成 attrs[key][0]
		String canonicalName = tokens.canonicalName;
		//根据PropertyTokenHolder获取属性对应的值
	...
	/**
	 * 解析表达式，把变量名、数组下标等一些信息封装成PropertyTokenHolder对象存起来
