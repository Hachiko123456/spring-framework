...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 * 2. {@link #extendInterceptors(List)} 方法，空实现
	 */
	...
	/**
	 * 从spring容器（包括父容器）中扫描已注册的MappedInterceptor.class类型的拦截器，添加到adaptedInterceptors中
	...
	/**
	 * 将interceptors初始化为HandlerInterceptor类型，添加到adaptedInterceptors中
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		...
		// 如果handler是String类型，则把它当做beanName去获取bean实例
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		...
		//遍历HandlerInterceptor
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			//如果是包含路劲匹配的拦截器
			if (interceptor instanceof MappedInterceptor) {
				MappedInterceptor mappedInterceptor = (MappedInterceptor) interceptor;
				//检查路劲是否匹配，路劲匹配则添加到执行链中的拦截器列表
				if (mappedInterceptor.matches(request)) {
					chain.addInterceptor(mappedInterceptor.getInterceptor());
				}
			}
			//非路劲匹配的拦截器，直接添加到执行链的拦截器列表中即可
			else {
	...
	/**
	 * 判断handler是否为CorsConfigurationSource的实例
