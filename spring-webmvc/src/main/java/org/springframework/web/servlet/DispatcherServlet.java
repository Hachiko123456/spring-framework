...
public class DispatcherServlet extends FrameworkServlet {
	...
	/**
	 * MultipartResolver对应的beanName
	 */
	public static final String MULTIPART_RESOLVER_BEAN_NAME = "multipartResolver";

	/** Well-known name for the LocaleResolver object in the bean factory for this namespace. */
	/**
	 * localeResolver对应的beanName
	 */
	public static final String LOCALE_RESOLVER_BEAN_NAME = "localeResolver";

	/** Well-known name for the ThemeResolver object in the bean factory for this namespace. */
	/**
	 * themeResolver 对应的beanName
	 */
	...
	/**
	 * 是否从祖先容器中查询{@link HandlerMapping} 实现类
	 */
	...
	/**
	 * FlashMap管理器，负责重定向时保存参数到临时存储（默认session）中
	 */
	@Nullable
	...
	/**
	 * LocaleResolver接口定义了Spring MVC如何获取客户端(浏览器)的地区，initLocaleResolver方法在容器中寻找此bean，
	 * 如果没有，注册AcceptHeaderLocaleResolver，即根据request的请求头Accept-Language获取地区。
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		//如果需要发现所有的HandlerMapping
		if (this.detectAllHandlerMappings) {
			// Find all HandlerMappings in the ApplicationContext, including ancestor contexts.
			//从当前容器和父容器中查找全部的HandlerMapping
	...
	/**
	 * 获取DispatcherServlet.properties文件的默认配置，并把这些配置解析成对应的类
	...
	protected void doService(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//如果request中包含属性javax.servlet.include.request_uri
		if (WebUtils.isIncludeRequest(request)) {
			...
			while (attrNames.hasMoreElements()) {
				...
				//保存key值为org.springframework.web.servlet
		...
		// 保存WebApplicationContext、LocaleResolver、ThemeResolver、ThemeSource 到request属性中
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				//检测请求是否为上传请求，如果是则通过multipartResolver将其封装成MultipartHttpServletRequest
				...
				/**
				 * 拦截器的前置处理，如果有一个拦截器返回false，则直接结束程序
				 */
				...
				// 调用适配器的handler方法，也就是执行对应的方法，并返回视图
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		//标记是否生成异常的ModelView对象
		boolean errorView = false;

		//请求发生异常
		if (exception != null) {
			//从ModelAndViewDefiningException获得ModelAndView对象
	...
	/**
	 * 如果请求是multipart请求，把请求转换成MultipartHttpServletRequest再返回
