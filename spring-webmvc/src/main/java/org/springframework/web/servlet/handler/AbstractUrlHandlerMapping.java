...
public abstract class AbstractUrlHandlerMapping extends AbstractHandlerMapping implements MatchableHandlerMapping {
	...
	protected Object getHandlerInternal(HttpServletRequest request) throws Exception {
		...
		if (handler == null) {
			...
			//如果是根路劲，使用rootHandler
			if (StringUtils.matchesCharacter(lookupPath, '/')) {
				rawHandler = getRootHandler();
			}
			//没有rootHandler或者不是根路径就使用defaultHandler
			if (rawHandler == null) {
				rawHandler = getDefaultHandler();
			}
			if (rawHandler != null) {
				// Bean name or resolved handler?
				// 如果是beanName,则从bean容器中获取bean实例
				...
				//把handler封装成HandlerExecutionChain返回
	...
	protected Object lookupHandler(String lookupPath, HttpServletRequest request) throws Exception {
		...
		if (bestMatch != null) {
			...
			// Bean name or resolved handler?
			// 如果处理器是String类型，则获取beanName对应的bean实例
			if (handler instanceof String) {
				String handlerName = (String) handler;
				handler = obtainApplicationContext().getBean(handlerName);
			}
	...
	protected Object buildPathExposingHandler(Object rawHandler, String bestMatchingPattern,
		...
		//PathExposingHandlerInterceptor负责把最佳路劲和当前匹配的路劲暴露到request中
		chain.addInterceptor(new PathExposingHandlerInterceptor(bestMatchingPattern, pathWithinMapping));
		if (!CollectionUtils.isEmpty(uriTemplateVariables)) {
			//UriTemplateVariablesHandlerInterceptor负责把路劲中的URL参数暴露到request中
	...
	public RequestMatchResult match(HttpServletRequest request, String pattern) {
		...
		//获得请求路劲，request.getAttribute(UrlPathHelper.class.getName() + ".path")
		String lookupPath = UrlPathHelper.getResolvedLookupPath(request);
		//模式匹配，若匹配，则返回RequestMatchResult对象
