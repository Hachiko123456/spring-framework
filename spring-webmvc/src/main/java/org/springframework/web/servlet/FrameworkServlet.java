...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	/**
	 * 创建的方式有4种
	 * 1. {@link #FrameworkServlet(WebApplicationContext)}
	 * 2. {@link ApplicationContextAware#setApplicationContext(ApplicationContext applicationContext)}
	 * 3. {@link #findWebApplicationContext()}
	 * 4. {@link #createWebApplicationContext(WebApplicationContext parent)}
	 */
	@Nullable
	private WebApplicationContext webApplicationContext;

	/**
	 * 标记是否是通过setApplicationContext注入的WebApplicationContext
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		if (this.webApplicationContext != null) {
			...
			// 如果是构造方法已经传入了webApplicationContext属性，则直接使用
			wac = this.webApplicationContext;
			//ConfigurableWebApplicationContext类型
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					//配置和初始化ConfigurableWebApplicationContext
		...
		//根据根容器创建一个WebApplicationContext对象
		if (wac == null) {
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		//添加监听器 SourceFilteringListener到wac中
	...
	protected void applyInitializers(ConfigurableApplicationContext wac) {
		//ContextLoader.GLOBAL_INITIALIZER_CLASSES_PARAM=globalInitializerClasses
		String globalClassNames = getServletContext().getInitParameter(ContextLoader.GLOBAL_INITIALIZER_CLASSES_PARAM);
		//检测globalInitializerClasses配置
		...
		//在容器初始化前调用ApplicationContextInitializer.initialize方法
	...
	/**
	 * 具体的实现在DispatcherServlet
	 * 触发有两种方式
	 * 1. 当this.refreshEventReceived为false时{@link FrameworkServlet#initWebApplicationContext()}
	 * 2. {@link ContextRefreshListener#onApplicationEvent(ContextRefreshedEvent)}
	...
	/**
	 * 因为HttpServlet没有处理PATCH请求，所以要覆盖service方法去处理PATCH请求
	...
	protected void doOptions(HttpServletRequest request, HttpServletResponse response)
		...
		// 调用父方法，并在响应头中的"Allow"中增加PATCH的值
	...
	/**
	 * 发布ServletRequestHandledEvent事件，但是SpringMVC并没有监听器监听该事件
	 * @param request
	 * @param response
	 * @param startTime
	 * @param failureCause
