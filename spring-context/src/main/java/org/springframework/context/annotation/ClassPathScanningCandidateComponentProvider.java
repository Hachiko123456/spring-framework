...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	/**
	 * 所有META-INF/spring.components文件的内容
	 */
	@Nullable
	...
	public Set<BeanDefinition> findCandidateComponents(String basePackage) {
		if (this.componentsIndex != null && indexSupportsIncludeFilters()) {
			/**
			 * 这里不会去扫描指定路劲下的.class文件，而是读取所有META-INF/spring.components文件中符合条件的类名
			 * 直接添加.class后缀就是编译文件，而不要去扫描
			 */
			return addCandidateComponentsFromIndex(this.componentsIndex, basePackage);
		}
		else {
			/**
			 * 扫描包路劲，通过asm解析出符合条件的ScannedGenericBeanDefinition们，并返回
			 * 首先需要去扫描指定路劲下所有的.class文件，该过程对于性能有不少的消耗
			 * 然后通过asm根据.class文件可以获取这个类的所有元信息，也就可以解析出对应的BeanDefinition对象
			 */
	...
	private Set<BeanDefinition> addCandidateComponentsFromIndex(CandidateComponentsIndex index, String basePackage) {
		...
		try {
			...
			//根据过滤器从所有META-INF/spring.components文件中获取所有符合条件的类名称
			for (TypeFilter filter : this.includeFilters) {
				...
				//获取注解（或类对应的条目），并过滤出basePackage包名下的条目（类的名称）
				types.addAll(index.getCandidateTypes(basePackage, stereotype));
			}
			boolean traceEnabled = logger.isTraceEnabled();
			boolean debugEnabled = logger.isDebugEnabled();
			//开始对过滤出来的类名进行处理，符合条件的类名会解析出一个AnnotatedGenericBeanDefinition
		...
		try {
			...
			for (Resource resource : resources) {
				...
				if (resource.isReadable()) {
					try {
						//根据这个类名找到.class文件，通过ASM获取这个类的所有信息
						...
						if (isCandidateComponent(metadataReader)) {
							...
							/**
							 * 再次判断这个类是否符合条件（不是内部类并且是一个具体类）
							 * 具体类：不是接口也不是抽象类，如果是抽象类则需要带有{@link Lookup}注解
							 */
							if (isCandidateComponent(sbd)) {
								if (debugEnabled) {
	...
	/**
	 * 判断类是否为一个具体类(非内部类、抽象类)，或者抽象类且方法被{@link Lookup}注解
