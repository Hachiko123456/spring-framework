...
public interface MergedAnnotations extends Iterable<MergedAnnotation<Annotation>> {
	...
	/**
	 * 是否有出现A注解（例如@Service被@Component注解，如果被@Service的类，调用isPresent(Component.class)则返回true）
	...
	/**
	 * 判断是否直接包含A注解（例如@Service被@Component注解，如果被@Service的类，调用isDirectlyPresent(Component.class)则返回false）
