...
public class ResolvableType implements Serializable {
	 ...
	 * 缓存数组泛型信息
	 * 例如List<String>[] list
	 * ResolvableType t=ResolvableType.forField(getClass().getDeclaredField("list)).getComponentType()
	 * t.getComponentType().type // java.util.List<java.lang.String>
	...
	public Class<?> getRawClass() {
		//如果type和resolved相等，则一定是Class类型
		if (this.type == this.resolved) {
			return this.resolved;
		}
		Type rawType = this.type;
		//如果是泛型类型，则获取泛型声明的rawType
		if (rawType instanceof ParameterizedType) {
			rawType = ((ParameterizedType) rawType).getRawType();
		}
		//否则就看是否为Class，是就强转为Class返回，否则就返回null
	...
	/**
	 * 返回ResolveType对应的Class类型，如果Class类型为空，默认用Object.class去代替
	...
	/**
	 * 类似于Class的isAssignableFrom的功能（class1.isAssignableFrom(class2)判断class2是否为class1的子类或者子接口）,
	 * Determine whether this {@code ResolvableType} is assignable from the
	 * specified other type.
	...
	/**
	 * 获取type对应的数组类型，然后再转为ResolvableType
	...
	/**
	 * 查找当前的ResolvableType对应的Class类型继承的父类和实现的接口是否有Collection类型
	...
	/**
	 * 查找当前的ResolvableType对应的Class类型继承的父类和实现的接口是否有Map类型
	...
	/**
	 * 根据type类型，查找当前ResolvableType对应的Class类型继承的父类和实现的接口的类型是否与其相等
	 * 相等则返回
	 * 找不到则返回NONE
	...
	/**
	 * 判断基础类型是否为无法通过变量解析器(variableResolver)解析的类型变量
	...
	/**
	 * 获取ResolveType的嵌套类型，0,1都是代表自身，从2开始表示嵌套的第一层
	...
	/**
	 * 嵌套获取泛型的类型信息，例如Map<String, Map<Character, Map<Boolean, List<Integer>>>>
	 * 如果是传参(1,1,1),则获取到的结果为List，第一个1是获取第二个Map,第二个1是获取第三个Map，第三个1获取的是第三个Map的List
	...
	public ResolvableType[] getGenerics() {
		...
		if (generics == null) {
			...
			//WildcardType、TypeVariable 类型调用 resolveType() 重新解析
			else {
	...
	/**
	 * 把泛型信息转为Class（如果resolve为空，则用fallback类型去替代）
	...
	/**
	 * 把Type类型转为Class类型，如果是Class类型的则直接返回，
	 * GenericArrayType类型的则获取该泛型数组成员变量的Class类型，
	 * ParameterizedType、WildcardType、TypeVariable三种比较复杂的类型则特殊处理
	 * @return
	 */
	@Nullable
	private Class<?> resolveClass() {
		...
		//数组类型（声明有[]则是GenericArrayType类型），该泛型数组成员变量的Class类型
		if (this.type instanceof GenericArrayType) {
			Class<?> resolvedComponent = getComponentType().resolve();
			return (resolvedComponent != null ? Array.newInstance(resolvedComponent, 0).getClass() : null);
		}
		//ParameterizedType、WildcardType、TypeVariable
		return resolveType().resolve();
	}

	/**
	 * 把Type包装为ResolvableType
	...
	ResolvableType resolveType() {
		...
		if (this.type instanceof ParameterizedType) {
			//用ParameterizedType的rawType，List<String>,rawType对应的java.util.List
			return forType(((ParameterizedType) this.type).getRawType(), this.variableResolver);
		}
		//通配符 ?
		if (this.type instanceof WildcardType) {
			//对应的是上界通配符，<? extends T>，获取T或者T的子类，如果有多个，则只取一个
			Type resolved = resolveBounds(((WildcardType) this.type).getUpperBounds());
			if (resolved == null) {
				//对应的下界通配符，<? super T>，获取T或T的父类，如果有多个，则只取一个
	...
	static ResolvableType forType(
		...
		if (type == null && typeProvider != null) {
			//通过动态代理根据TypeProvider.getType()类型去生成Type实例
		...
		//如果是Class类型，直接返回，因为用Class构造ResolvableType的代价很小，不用缓存
		...
		if (cachedType == null) {
			//其他类型的话就要进行缓存，因为要获取该类型对应的Class类型，需要调用resolveClass()
			//在该方法中对GenericArrayType、ParameterizedType、WildcardType、TypeVariable四种类型进行一个遍历，
			// 然后再调用forType方法获取ResolveType，才获取到的Class类型
			//步骤很多，代价很大，所以用一层缓存把结果缓存起来
	...
	private static class TypeVariablesVariableResolver implements VariableResolver {
		...
		/***
		 * 返回相同TypeVariable的ResolvableType
		 * @param variable
