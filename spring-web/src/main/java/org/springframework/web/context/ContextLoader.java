...
public class ContextLoader {
	...
	/**
	 * 用来保存当前ClassLoader类加载器与WebApplicationContext对象的映射关系
	 * 如果当前线程的类加载器不等于ContextLoader.class的类加载器的时候就会把类加载器和WebApplicationContext这层关系给映射起来
	...
	/**
	 * 如果当前线程的类加载器就是ContextLoader类所在的类加载器，则该属性用于保存WebApplicationContext对象
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		//若已经存在WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE对应的WebApplicationContext对象，则抛出异常
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 无父容器，则进行加载和配置（当前类的loadParentContext返回为null）
			...
			//把rootContext设置到WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE属性中
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		//获取contextClass属性的类型（默认情况是从ContextLoader.properties配置文件中读取，为XmlWebApplicationContext）
		Class<?> contextClass = determineContextClass(sc);
		//如果不是ConfigurableWebApplicationContext则抛出异常
	...
	/**
	 * 看是否有定义contextClass的类型，默认为XmlWebApplicationContext
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		//看是否有自定义contextClass,一般情况下都没有定义
		...
		if (contextClassName != null) {
			...
			//没有定义则用默认的XmlWebApplicationContext
	...
	/**
	 * 获取自定义的ApplicationContextInitializer，在刷新Web容器之前执行ApplicationContextInitializer的initialize方法
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		...
		//执行ApplicationContextInitializer的initializer方法
	 ...
	 * 获取globalInitializerClasses、contextInitializerClasses两个的属性配置
	...
	protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>>
		...
		//查找globalInitializerClasses属性配置
		...
		//查找contextInitializerClasses属性配置
