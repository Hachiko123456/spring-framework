...
public class RequestParamMethodArgumentResolver extends AbstractNamedValueMethodArgumentResolver
	...
	public boolean supportsParameter(MethodParameter parameter) {
		...
		if (parameter.hasParameterAnnotation(RequestParam.class)) {
			...
			//TODO 平时没有指定name属性的都是用RequestParamMapMethodArgumentResolver去处理的
			...
			//有@RequestPart注解则返回false
			if (parameter.hasParameterAnnotation(RequestPart.class)) {
				return false;
			}
			parameter = parameter.nestedIfOptional();
			//Multipart或者Part类型，或者是它们的数组类型，则返回true
			if (MultipartResolutionDelegate.isMultipartArgument(parameter)) {
				return true;
			}
			//如果开始useDefaultResolution，则判断是否为基本类型
	...
	/**
	 * 根据@RequestParam去创建NamedValueInfo对象
	...
	protected Object resolveName(String name, MethodParameter parameter, NativeWebRequest request) throws Exception {
		//把request转为HttpServletRequest
		HttpServletRequest servletRequest = request.getNativeRequest(HttpServletRequest.class);

		//处理MultipartFile和Part的情况
